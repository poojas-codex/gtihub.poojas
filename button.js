...
//        appendNode(node, newtabBtn);
        ...
        // add download button
        let downloadBtn = createCustomBtn(svgDownloadBtn, iconColor, 'download-btn', '16px');
...
//            onMouseInHandler({ currentTarget: newtabBtn });
    ...
    function createCustomBtn(svg, iconColor, className, marginLeft) {
        ...
        newBtn.setAttribute('style', 'cursor: pointer;margin-left: ' + marginLeft + ';margin-top: 2px;z-index: 999;');
...
//        a.remove();
    ...
    function forceDownload(blob, filename, extension) {
        ...
        a.click();
        a.remove();
//        Android.onDownloadButtonClick(a);
    }
    ...
    function downloadResource(url, filename) {
        ...
        console.log(`Dowloading ${url}`);
                Android.onDownloadButtonClick(url);
        // ref: https://stackoverflow.com/questions/49474775/chrome-65-blocks-cross-origin-a-download-client-side-workaround-to-force-down
        if (!filename) {
            filename = url
                .split('\\')
        ...
        fetch(url, {
            ...
            .then(blob => {
                ...
                forceDownload(url, filename, extension);
            })
